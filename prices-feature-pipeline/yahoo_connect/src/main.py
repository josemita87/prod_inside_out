...
class YahooDataFetcher:
    """Class to fetch and process data from Yahoo Finance."""
            ...
            try:
                offset = prices.loc[
                    prices['ticker'] == ticker][
                        'date'
                    ].max() if not prices.empty else None
                ...
                all_prices = pd.concat(
                    [all_prices, new_data], axis=0
                ) if not all_prices.empty else new_data
    ...
    def process_tickers(self)-> None:
        ...
        if config.system_inference:
            ...
            data = self.feature_store.fetch_BT4(
        ...
        for i in tqdm(
            range(0, len(tickers), config.buffer_size), 
