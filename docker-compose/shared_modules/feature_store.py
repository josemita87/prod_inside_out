...
class Connection:
    ...
    WRITE_OPTIONS = {'start_offline_materialization': True, 'mode': 'append'}
    ...
    def __init__(self) -> None:
        ...
        self.BT4 = self.fs.get_or_create_feature_group(
            name='bt4', version=config.feature_group_version, primary_key=['key'], event_time='date'
        )
    ...
    def push_P(self, data):
        ...
        if not data.empty:
            self.P.insert(features=data, write_options=self.WRITE_OPTIONS)

    def fetch_BT4(self, key, value):
        BT4 = self.BT4
        if key and value:
            BT4 = BT4.filter(BT4[key] == value)
        return BT4.read(read_options={'use_hive': True})

    def push_BT4(self, data):
        if not data.empty:
            self.BT4.insert(features=data, write_options=self.WRITE_OPTIONS)
    ...
    def push_RT4(self, data):
        if not data.empty:
            self.RT4.insert(features=data, write_options=self.WRITE_OPTIONS)
    ...
    def push_BI4(self, data):
        if not data.empty:
            self.BI4.insert(features=data, write_options=self.WRITE_OPTIONS)
    ...
    def push_BIR4(self, data):
        if not data.empty:
            self.BIR4.insert(features=data, write_options=self.WRITE_OPTIONS)
