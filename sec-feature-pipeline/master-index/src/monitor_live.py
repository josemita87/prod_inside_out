"""Selenium-based monitor that scrapes live Form 4 filing links from SEC EDGAR."""
...
class SECLinkMonitor:
    """Scrape recent Form 4 filing links from the SEC EDGAR current-filings page.

    Attributes:
        url: EDGAR current-filings URL queried for Form 4 filings.
        num_pages: Number of paginated result pages to scrape.
        headers: HTTP headers sent with the browser requests.
    """

    def __init__(self, num_pages: int):
        """Initialize the monitor with the number of result pages to scrape.
    ...
    def _random_human_interaction(self, driver):
        ...
        Args:
            driver: Active WebDriver instance to drive the interactions.
        """
        # Random scrolling
    ...
    def parse_links(self, wait_time: int = 10) -> set:
        """Scrape Form 4 filing links across the configured number of pages.

        Args:
            wait_time: Maximum seconds to wait for page elements to load.

        Returns:
            A set of relative ``.txt`` filing link paths collected from the pages.
        """
